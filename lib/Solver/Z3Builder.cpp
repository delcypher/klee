 ...
 */
Z3_ast Z3Builder::getFalse() { return Z3_mk_false(ctx); }

Z3_ast Z3Builder::bvOne(unsigned width) { return bvZExtConst(width, 1); }

Z3_ast Z3Builder::bvZero(unsigned width) { return bvZExtConst(width, 0); }
...
Z3_ast Z3Builder::eqExpr(Z3_ast a, Z3_ast b) { return Z3_mk_eq(ctx, a, b); }
...
Z3_ast Z3Builder::bvLeftShift(Z3_ast expr, unsigned shift) {
  ...
  if (shift == 0) {
    ...
    return Z3_mk_concat(ctx, bvExtract(expr, width - shift - 1, 0),
                        bvZero(shift));
  }
}
...
Z3_ast Z3Builder::bvMinusExpr(unsigned width, Z3_ast minuend,
                              Z3_ast subtrahend) {
  return Z3_mk_extract(ctx, width - 1, 0,
                       Z3_mk_bvsub(ctx, minuend, subtrahend));
}
...
Z3_ast Z3Builder::bvMultExpr(unsigned width, Z3_ast multiplacand,
                             Z3_ast multiplier) {
  return Z3_mk_extract(ctx, width - 1, 0,
                       Z3_mk_bvmul(ctx, multiplacand, multiplier));
}
...
Z3_ast Z3Builder::notExpr(Z3_ast expr) { return Z3_mk_not(ctx, expr); }

Z3_ast Z3Builder::bvNotExpr(Z3_ast expr) { return Z3_mk_bvnot(ctx, expr); }
...
Z3_ast Z3Builder::constructAShrByConstant(Z3_ast expr, unsigned shift,
                                          Z3_ast isSigned) {
  unsigned width = getBVLength(expr);
  ...
  if (shift == 0) {
    ...
    return iteExpr(isSigned, Z3_mk_concat(ctx, bvMinusOne(shift),
                                          bvExtract(expr, width - 1, shift)),
                   bvRightShift(expr, shift));
  }
}

Z3_ast Z3Builder::constructMulByConstant(Z3_ast expr, unsigned width,
                                         uint64_t x) {
  uint64_t add, sub;
  Z3_ast res = 0;
 ...
 */
Z3_ast Z3Builder::constructUDivByConstant(Z3_ast expr_n, unsigned width,
                                          uint64_t d) {
 ...
 * @return n/d without doing explicit division
 */
Z3_ast Z3Builder::constructSDivByConstant(Z3_ast expr_n, unsigned width,
                                          uint64_t d) {
...
Z3_ast Z3Builder::getArrayForUpdate(const Array *root, const UpdateNode *un) {
  if (!un) {
...
Z3_ast Z3Builder::construct(ref<Expr> e, int *width_out) {
  if (!UseConstructHashZ3 || isa<ConstantExpr>(e)) {
    ...
    ExprHashMap<std::pair<Z3_ast, unsigned> >::iterator it = constructed.find(e);
...
Z3_ast Z3Builder::constructActual(ref<Expr> e, int *width_out) {
  ...
  case Expr::Xor: {
    ...
    if (*width_out == 1) {
      ...
      return iteExpr(left, Z3_ast(notExpr(right)), right);
    } else {
